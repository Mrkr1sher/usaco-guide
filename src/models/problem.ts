  ...
  if (solutionMetadata.kind === 'autogen-label-from-site') {
    ...
    sol = {
      ...
      url:
        ...
        solutionMetadata.usacoId, //removed id_to_sol
    };
...
export class Problem {
  ...
  private tryAutoGeneratingSolution() {
    ...
    // if (isUsaco(this.source) && this.id in id_to_sol) { //removed id_to_sol
    if (true) {  
    autoGeneratedSolURL =
        `http://www.usaco.org/current/data/` + this.id; //removed id_to_sol
